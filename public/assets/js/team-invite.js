...
const STATUS = { PENDING_ACCOUNT: 'Menunggu akun', PENDING_DECLARATION: 'Menunggu deklarasi', CLEARED: 'Deklarasi selesai — menunggu aktivasi tim', CONFLICT: 'Benturan kepentingan dilaporkan', REVOKED: 'Penunjukan dicabut' };
...
function render(items){nominationList.replaceChildren();if(!items.length){nominationList.append(el('div','Tidak ada penunjukan Tim Pemeriksa aktif untuk akun ini.','form-card'));return;}
 for(const item of items){const card=el('section',null,'form-card');const head=el('div',null,'status-head');head.append(el('h2',item.nomorLaporan||'Penunjukan Tim Pemeriksa'),el('span',STATUS[item.nominationStatus]??item.nominationStatus,'status-badge'));card.append(head);
 const summary=el('div',null,'summary-list');[['Peran',ROLE[item.committeeRole]??item.committeeRole],['Kategori',item.memberCategory],['Klasifikasi',item.classification==='INTEGRITY'?'Pelanggaran Integritas':(item.classification??'—')],['Alasan dipilih',item.rationale||'—'],['Konteks conflict check',item.conflictContext||'—']].forEach(([k,v])=>{const row=el('div');row.append(el('dt',k),el('dd',v));summary.append(row)});card.append(summary);
