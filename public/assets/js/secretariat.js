...
async function invoke(body){const requestBody=isUat?{...body,uat:true}:body;const{data,error}=await supabaseClient.functions.invoke('secretariat-team-action',{body:requestBody});if(error){let detail=error.message;try{const c=await error.context?.json();if(c?.error)detail=c.error;}catch(_){}throw new Error(detail||'Aksi belum dapat diproses.');}return data;}
