...
async function invoke(body){const requestBody=isUat?{...body,uat:true}:body;const fn=TEAM_MUTATION_ACTIONS.has(String(body.action??''))?'secretariat-team-mutation':'secretariat-team-action';const{data,error}=await supabaseClient.functions.invoke(fn,{body:requestBody});if(error){let detail=error.message;try{const c=await error.context?.json();if(c?.error)detail=c.error;}catch(_){}throw new Error(detail||'Aksi belum dapat diproses.');}return data;}
