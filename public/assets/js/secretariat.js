...
const TEAM_MUTATION_ACTIONS=new Set(['ADD_MEMBER','REVOKE_MEMBER','ACTIVATE_TEAM']);
const REVIEW_REMEDIATION_MUTATION_ACTIONS=new Set(['REVIEW_FINDINGS','ADD_REMEDIATION_ACTION','COMPLETE_REMEDIATION_ACTION','WAIVE_REMEDIATION_ACTION']);
...
async function invoke(body){const requestBody=isUat?{...body,uat:true}:body;const action=String(body.action??'');const fn=TEAM_MUTATION_ACTIONS.has(action)?'secretariat-team-mutation':REVIEW_REMEDIATION_MUTATION_ACTIONS.has(action)?'secretariat-review-remediation-mutation':'secretariat-team-action';const{data,error}=await supabaseClient.functions.invoke(fn,{body:requestBody});if(error){let detail=error.message;try{const c=await error.context?.json();if(c?.error)detail=c.error;}catch(_){}throw new Error(detail||'Aksi belum dapat diproses.');}return data;}
...
async function loadCases(){showMessage(isUat?'Mode UAT aktif — memuat data test Sekretariat.':'');const data=await invoke({action:'LIST'});if(isUat&&data?.uat!==true){throw new Error('Mode UAT tidak diakui server. Muat ulang halaman setelah deploy terbaru.');}cases=data.cases??[];renderCaseList();if(selectedCaseId&&cases.some(c=>c.id===selectedCaseId))await loadDetail(selectedCaseId);else if(cases.length)await loadDetail(cases[0].id);else caseDetail.replaceChildren(el('p',isUat?'Tidak ada kasus UAT aktif di bawah kewenangan Sekretariat DS.':'Tidak ada kasus aktif di bawah kewenangan Sekretariat DS.','empty-state'));}
function renderCaseList(){caseList.replaceChildren();if(!cases.length)return caseList.append(el('p',isUat?'Tidak ada kasus UAT aktif.':'Tidak ada kasus aktif.','empty-state'));for(const c of cases){const b=el('button',null,`case-list-button${c.id===selectedCaseId?' active':''}`);b.type='button';b.append(el('strong',c.public_case_id),el('span',c.title),el('span',STATUS_LABELS[c.status]??c.status));if(c.is_test_data)b.append(el('span',`UAT${c.test_label?` · ${c.test_label}`:''}`,'status-badge'));b.addEventListener('click',()=>loadDetail(c.id));caseList.append(b);}}
async function loadDetail(caseId){selectedCaseId=caseId;renderCaseList();showMessage(isUat?'Mode UAT aktif — data test Sekretariat ditampilkan.':'');const data=await invoke({action:'DETAIL',caseId});renderDetail(data);}
...
function renderDetail(data){
 ...
 const meta=el('div',null,'case-meta');[['Nomor Laporan',c.public_case_id],['Pelapor',c.reporting_mode==='ANONYMOUS'?'Tanpa Identitas':'Identitas Dirahasiakan'],['Klasifikasi','Pelanggaran Integritas'],['Diterima',fmtDate(c.submitted_at)]].forEach(([k,v])=>{const x=el('div');x.append(el('span',k),el('strong',v));meta.append(x);});head.append(meta);if(c.is_test_data){const note=el('div',null,'notice');note.append(el('strong','Data UAT / Test'),el('p',c.test_label||'Case ini ditandai sebagai data pengujian.'));head.append(note);}caseDetail.append(head);
...
document.querySelector('#googleLogin')?.addEventListener('click',async()=>{const redirectTo=new URL(`secretariat.html${location.search}`,window.location.href).href;const{error}=await supabaseClient.auth.signInWithOAuth({provider:'google',options:{redirectTo}});if(error){loginMessage.textContent=error.message;loginMessage.hidden=false;}});
