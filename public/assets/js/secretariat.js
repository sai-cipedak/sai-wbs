...
async function loadDetail(caseId){selectedCaseId=caseId;renderCaseList();showMessage('');const data=await invoke({action:'DETAIL',caseId});renderDetail(data.case,data.report,data.messages??[],data.team??[],data.allegations??[],data.findings??[],data.reviews??[]);}

function renderDetail(c,report,messages,team,allegations,findings,reviews){
 ...
 const comm=el('section',null,'case-section');comm.append(el('h3','Komunikasi dengan pelapor'));if(!messages.length)comm.append(el('p','Belum ada pesan.','muted'));messages.forEach(m=>{const box=el('div',null,'message-item');box.append(el('strong',m.sender_type==='REPORTER'?'Pelapor':'Tim Penanganan'),el('p',m.body),el('small',fmtDate(m.created_at)));comm.append(box);});caseDetail.append(comm);
 ...
 const teamSection=el('section',null,'case-section');const teamHead=el('div',null,'status-head');teamHead.append(el('h3','Tim Pemeriksa'),el('span',`${team.filter(m=>m.nomination_status==='CLEARED'&&['CASE_LEAD','INVESTIGATOR'].includes(m.committee_role)).length} cleared`,'status-badge'));teamSection.append(teamHead);
