alter table public.case_followups add column if not exists owner_authority_code text;

update public.case_followups f
set owner_authority_code = c.authority_code
from public.cases c
where c.id=f.case_id and f.owner_authority_code is null;

alter table public.case_followups alter column owner_authority_code set not null;
create index if not exists case_followups_owner_status_due_idx on public.case_followups(owner_authority_code,status,due_at);
...
create or replace function public.complete_case_followup(
  p_followup_id uuid,p_actor_user_id uuid,p_organization_id uuid,p_check_method text,p_outcome text,p_risk_level text,p_notes text,p_escalation_note text default null
) returns jsonb
language plpgsql
security definer
set search_path='public','pg_temp'
...
begin
  ...
  v_required_role := case v_followup.owner_authority_code when 'GRIEVANCE' then 'GRIEVANCE_COORDINATOR' else v_followup.owner_authority_code end;
  if not exists(
    select 1 from public.user_system_roles r
    where r.user_id=p_actor_user_id and r.organization_id=p_organization_id and r.role_code=v_required_role
      and r.active_from<=v_now and (r.active_until is null or r.active_until>v_now)
  ) then raise exception 'FOLLOWUP_FORBIDDEN'; end if;
  ...
  if p_outcome='RETALIATION_CONCERN' then
    ...
    insert into public.cases(organization_id,public_case_id,reporting_mode,status,classification,priority,authority_code,policy_version_id,created_by_user_id,submitted_at,created_at,updated_at)
    values(v_case.organization_id,v_linked_public_id,v_case.reporting_mode,'UNDER_REVIEW','INTEGRITY',case when p_risk_level='CRITICAL' then 'CRITICAL' else 'HIGH' end,'TRIAGE',v_case.policy_version_id,p_actor_user_id,v_now,v_now,v_now)
    ...
    values(v_linked_case_id,'Tindak Lanjut Retaliation — '||v_case.public_case_id,
      'Case ini dibuat otomatis dari follow-up '||v_followup.day_offset||' hari atas '||v_case.public_case_id||'.'||chr(10)||chr(10)||
      'Temuan follow-up: '||trim(p_notes)||chr(10)||chr(10)||'Catatan eskalasi: '||trim(p_escalation_note),false,false,v_now);
    ...
    insert into public.audit_logs(organization_id,case_id,actor_user_id,event_type,object_type,object_id,details)
    values
      (p_organization_id,v_case.id,p_actor_user_id,'FOLLOWUP_RETALIATION_CASE_CREATED','case_followup',p_followup_id,jsonb_build_object('linked_case_id',v_linked_case_id,'linked_public_case_id',v_linked_public_id,'day_offset',v_followup.day_offset,'owner_authority_code',v_followup.owner_authority_code)),
    ...
    v_owner_text := case v_followup.owner_authority_code when 'DEKOM' then 'Dekom' when 'HSE' then 'Otoritas Perlindungan' when 'GRIEVANCE' then 'Koordinator Pengaduan' else 'Sekretariat DS' end;

    insert into public.case_remediation_actions(case_id,action_text,owner_text,status,created_by,created_at,updated_at)
    values(v_case.id,left('Remediation dibuka kembali karena follow-up '||v_followup.day_offset||' hari menunjukkan tindak lanjut sebelumnya tidak efektif/gagal. '||trim(p_notes),5000),v_owner_text,'PENDING',p_actor_user_id,v_now,v_now)
    returning id into v_remediation_id;

    update public.case_followups set status='CANCELLED',updated_at=v_now where closure_id=v_followup.closure_id and id<>p_followup_id and status='SCHEDULED';
    update public.case_followups set escalation_status='RESOLVED',escalation_resolved_by=p_actor_user_id,escalation_resolved_at=v_now,
      escalation_resolution_note='Remediation pada case asal dibuka kembali secara otomatis.',escalation_resolution_mode='REMEDIATION_REOPENED' where id=p_followup_id;
    insert into public.case_messages(case_id,sender_type,sender_user_id,body,visible_to_reporter,created_at)
    ...
    insert into public.audit_logs(organization_id,case_id,actor_user_id,event_type,object_type,object_id,details)
    values(p_organization_id,v_case.id,p_actor_user_id,'CASE_REMEDIATION_REOPENED','case_remediation_action',v_remediation_id,jsonb_build_object('source_followup_id',p_followup_id,'day_offset',v_followup.day_offset,'owner_authority_code',v_followup.owner_authority_code));
  end if;

  insert into public.audit_logs(organization_id,case_id,actor_user_id,event_type,object_type,object_id,details)
  values(p_organization_id,v_case.id,p_actor_user_id,'FOLLOWUP_COMPLETED','case_followup',p_followup_id,
    jsonb_build_object('day_offset',v_followup.day_offset,'outcome',p_outcome,'risk_level',p_risk_level,'completed_early',v_followup.due_at>v_now,'escalation_required',v_escalation_required,'linked_case_id',v_linked_case_id,'owner_authority_code',v_followup.owner_authority_code));

  return jsonb_build_object('ok',true,'followupId',p_followup_id,'escalationOpen',case when p_outcome='REMEDIATION_FAILURE' then false else v_escalation_required end,
    'linkedCaseId',v_linked_case_id,'linkedPublicCaseId',v_linked_public_id,'remediationReopened',p_outcome='REMEDIATION_FAILURE',
    'caseStatus',case when p_outcome='REMEDIATION_FAILURE' then 'REMEDIATION' else 'CLOSED' end,'ownerAuthorityCode',v_followup.owner_authority_code);
end;
$$;

create or replace function public.resolve_case_followup_escalation(p_followup_id uuid,p_actor_user_id uuid,p_organization_id uuid,p_resolution_note text)
returns jsonb
language plpgsql
security definer
set search_path='public','pg_temp'
...
begin
  ...
  update public.case_followups set escalation_status='RESOLVED',escalation_resolved_by=p_actor_user_id,escalation_resolved_at=v_now,
    escalation_resolution_note=trim(p_resolution_note),escalation_resolution_mode=case when v_followup.outcome='RETALIATION_CONCERN' then 'LINKED_CASE' else 'MANUAL_RESOLUTION' end,updated_at=v_now
  where id=p_followup_id;

  insert into public.audit_logs(organization_id,case_id,actor_user_id,event_type,object_type,object_id,details)
  values(p_organization_id,v_case.id,p_actor_user_id,'FOLLOWUP_ESCALATION_RESOLVED','case_followup',p_followup_id,
    jsonb_build_object('day_offset',v_followup.day_offset,'linked_case_id',v_followup.linked_case_id,'owner_authority_code',v_followup.owner_authority_code));

  return jsonb_build_object('ok',true,'linkedCaseId',v_followup.linked_case_id,'ownerAuthorityCode',v_followup.owner_authority_code);
end;
$$;
