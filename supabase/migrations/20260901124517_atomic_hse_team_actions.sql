create or replace function public.hse_add_case_team_member_atomic(
  p_case_id uuid,
...
begin
  ...
  insert into public.audit_logs(
    ...
    jsonb_build_object('committee_role',v_member.committee_role,'member_category',v_member.member_category,'nomination_status',v_member.nomination_status,'authority_code','HSE')
  );
...
create or replace function public.hse_activate_case_team_atomic(
  p_case_id uuid,
  p_actor_user_id uuid,
  p_organization_id uuid
) returns jsonb
...
declare
  v_case public.cases%rowtype;
  v_assessment public.case_safeguarding_assessments%rowtype;
  v_user_count integer;
  v_missing_assignments integer;
  v_has_lead boolean;
  v_protective_count integer := 0;
  v_now timestamptz := now();
begin
  select * into v_case from public.cases where id=p_case_id for update;
  if not found or v_case.organization_id<>p_organization_id then raise exception 'CASE_NOT_FOUND'; end if;
  ...
  from public.case_safeguarding_assessments
  where case_id=p_case_id
  order by assessed_at desc
  limit 1;
  if not found then raise exception 'SAFEGUARDING_ASSESSMENT_REQUIRED'; end if;

  if v_assessment.immediate_danger then
    select count(*) into v_protective_count from public.case_protective_actions where case_id=p_case_id;
    if v_protective_count<1 then raise exception 'PROTECTIVE_ACTION_REQUIRED'; end if;
  end if;
  ...
  insert into public.audit_logs(
    ...
    p_organization_id,p_case_id,p_actor_user_id,'TEAM_ACTIVATED','case',p_case_id,
    jsonb_build_object('authority_code','HSE','investigator_count',v_user_count,'previous_status','COMMITTEE_FORMATION','next_status','INVESTIGATION')
  );
...
revoke all on function public.hse_add_case_team_member_atomic(uuid,uuid,uuid,text,text,text,text,text,text) from public, anon, authenticated;
revoke all on function public.hse_revoke_case_team_member_atomic(uuid,uuid,uuid,uuid) from public, anon, authenticated;
revoke all on function public.hse_activate_case_team_atomic(uuid,uuid,uuid) from public, anon, authenticated;
grant execute on function public.hse_add_case_team_member_atomic(uuid,uuid,uuid,text,text,text,text,text,text) to service_role;
grant execute on function public.hse_revoke_case_team_member_atomic(uuid,uuid,uuid,uuid) to service_role;
grant execute on function public.hse_activate_case_team_atomic(uuid,uuid,uuid) to service_role;