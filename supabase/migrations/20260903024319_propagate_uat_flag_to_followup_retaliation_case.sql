...
begin
  ...
  if p_outcome='RETALIATION_CONCERN' then
    ...
    insert into public.cases(organization_id,public_case_id,reporting_mode,status,classification,priority,authority_code,policy_version_id,created_by_user_id,submitted_at,created_at,updated_at,is_test_data,test_label)
    values(v_case.organization_id,v_linked_public_id,v_case.reporting_mode,'UNDER_REVIEW','INTEGRITY',case when p_risk_level='CRITICAL' then 'CRITICAL' else 'HIGH' end,'TRIAGE',v_case.policy_version_id,p_actor_user_id,v_now,v_now,v_now,v_case.is_test_data,case when v_case.is_test_data then coalesce(nullif(v_case.test_label,''),'UAT')||' / RETALIATION FOLLOW-UP' else null end)
