create or replace function public.review_integrity_findings_atomic(
  p_case_id uuid,
...
begin
  ...
  if not found or v_case.organization_id<>p_organization_id or v_case.authority_code<>'SECRETARIAT' then raise exception 'CASE_NOT_FOUND'; end if;
  if v_case.status<>'AUTHORITY_REVIEW' then raise exception 'CASE_CHANGED'; end if;
  if v_case.classification<>'INTEGRITY' then raise exception 'INVALID_CLASSIFICATION'; end if;
  ...
  values(p_organization_id,p_case_id,p_actor_user_id,case when p_decision='APPROVED' then 'FINDINGS_APPROVED' else 'FINDINGS_RETURNED_FOR_REVISION' end,'case_authority_review',v_review_id,jsonb_build_object('next_status',v_next_status));
  return jsonb_build_object('ok',true,'status',v_next_status,'reviewId',v_review_id);
end;
$$;

create or replace function public.add_remediation_action_atomic(
  p_case_id uuid,
  p_actor_user_id uuid,
  p_organization_id uuid,
  p_action_text text,
...
begin
  ...
  insert into public.case_remediation_actions(case_id,action_text,owner_text,due_date,created_by)
  values(p_case_id,trim(p_action_text),v_owner,p_due_date,p_actor_user_id) returning id into v_id;
  insert into public.audit_logs(organization_id,case_id,actor_user_id,event_type,object_type,object_id,details)
  values(p_organization_id,p_case_id,p_actor_user_id,'REMEDIATION_ACTION_ADDED','case_remediation_action',v_id,'{}'::jsonb);
  return jsonb_build_object('ok',true,'remediationId',v_id);
end;
$$;

create or replace function public.finish_remediation_action_atomic(
  p_case_id uuid,
...
begin
  ...
  insert into public.audit_logs(organization_id,case_id,actor_user_id,event_type,object_type,object_id,details)
  values(p_organization_id,p_case_id,p_actor_user_id,case when p_final_status='COMPLETED' then 'REMEDIATION_ACTION_COMPLETED' else 'REMEDIATION_ACTION_WAIVED' end,'case_remediation_action',p_remediation_id,'{}'::jsonb);
  return jsonb_build_object('ok',true,'status',p_final_status,'remediationId',p_remediation_id);
end;
$$;

revoke all on function public.review_integrity_findings_atomic(uuid,uuid,uuid,text,text) from public, anon, authenticated;
revoke all on function public.add_remediation_action_atomic(uuid,uuid,uuid,text,text,date) from public, anon, authenticated;
revoke all on function public.finish_remediation_action_atomic(uuid,uuid,uuid,uuid,text,text) from public, anon, authenticated;
grant execute on function public.review_integrity_findings_atomic(uuid,uuid,uuid,text,text) to service_role;
grant execute on function public.add_remediation_action_atomic(uuid,uuid,uuid,text,text,date) to service_role;
grant execute on function public.finish_remediation_action_atomic(uuid,uuid,uuid,uuid,text,text) to service_role;
