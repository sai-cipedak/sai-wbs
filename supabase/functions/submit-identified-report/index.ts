...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: createdCase, error: caseError } = await admin.from('cases').insert({ organization_id: org.id, public_case_id: publicCaseId, reporting_mode: 'IDENTIFIED', status: safetyFastLane ? 'REFERRED_SAFEGUARDING' : 'SUBMITTED', classification: safetyFastLane ? 'SAFEGUARDING' : null, priority: safetyFastLane ? 'CRITICAL' : null, authority_code: safetyFastLane ? 'HSE' : 'TRIAGE', policy_version_id: org.active_policy_version_id, created_by_user_id: user.id, is_test_data: false }).select('id, submitted_at').single();
    ...
    await admin.from('audit_logs').insert({ organization_id: org.id, case_id: createdCase.id, actor_user_id: user.id, event_type: 'CASE_SUBMITTED_IDENTIFIED', object_type: 'case', object_id: createdCase.id, details: { safety_fast_lane: safetyFastLane, profile_member_type_preserved: profileMemberType === 'INTERNAL' } });

    return jsonResponse({ nomorLaporan: publicCaseId, status: safetyFastLane ? 'Sedang Ditangani' : 'Laporan Diterima', submittedAt: createdCase.submitted_at, identityProtection: 'Identitas Anda disimpan terpisah dan tersembunyi dari Tim Pemeriksa secara default.' }, 201, corsHeaders);
  } catch (error) {
